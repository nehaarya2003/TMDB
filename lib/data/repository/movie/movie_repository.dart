...
class MovieRepository extends IMovieAuthRepository {
  ...
  @override
  Future<AuthResponseModel?> authenticateAccount() async {
    try {
      // Make a GET request to the API endpoint to fetch user data.
      final response = await _dio.authenticateUser();
  ...
  @override
  Future<MovieListResponseModel?> discoverMovies() async {
    try {
      // Make a GET request to the API endpoint to fetch user data.
      final response = await _dio.fetchMovieList();
